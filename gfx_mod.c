...
Ander & Zech modified:
- Blocked event triggers from pressing keyboard keys
- Added some more fonts
- Added cursor changing
*/
...
char gfx_wait()
{
	...
	while(1) {
		...
		if(event.type==KeyPress) {//Commented out to prevent keyboard presses to trigger button click events
...
void gfx_cursor()
{
	int cursor_shape = XC_hand2;
	Cursor cursor;
	cursor = XCreateFontCursor(gfx_display, cursor_shape);
	XDefineCursor(gfx_display, gfx_window, cursor);
}